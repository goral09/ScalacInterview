object Test {
  ...
  val (questionFigures, questionBoard) = (List(King(), King(), Bishop(), Bishop(), Queen(), Queen(), Knight()), Board()(Size(7,7)))
                                                  //> questionFigures  : List[Product with Serializable with Figure] = List(K, K, 
                                                  //| B, B, Q, Q, N)
                                                  //| questionBoard  : Board = Board@144fc66
	val (ex1Figures, ex1Board) = (List(King(), King(), Rook()), Board()(Size(3,3)))
                                                  //> ex1Figures  : List[Product with Serializable with Figure] = List(K, K, R)
                                                  //| ex1Board  : Board = Board@18ad742
  ...
  def solve(figuresLeft: List[Figure], board: Board): List[List[(Figure, (Int, Int))]] = {
    ...
    tmpSolutions
                                                  ...
                                                  //| ))]]
  solve(questionFigures, questionBoard).size      //> res0: Int = 105630
...
class Board(val figures: List[(Figure, (Int, Int))])(implicit boardSize: Size) extends TilesGuarded {
  ...
  def emptyTiles(figure: Figure) = {
  	...
  	val (startPosX, startPosY) = lastFg match {
  		case Some((fig, posi)) => (posi._1, posi._2 + 1)
  		case None => (1,1)
  	}
